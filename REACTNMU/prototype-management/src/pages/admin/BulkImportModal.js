...
import { faDownload, faUpload, faTimes, faSave, faSpinner } from '@fortawesome/free-solid-svg-icons';
...
const BulkImportModal = ({ show, onHide, fetchUsers }) => {
  ...
  const downloadTemplate = async () => {
    try {
      ...
      const response = await api.get('/admin/download_users_template/', {
        responseType: 'blob'
      });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await api.post('/admin/users_import/', formData, {
        headers: {
